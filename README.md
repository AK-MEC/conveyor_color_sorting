...
| `SETTLE_MIN_ACTIVE_TIME`| 3.0 s  | Minimum time on belt before settle check (prevents false trigger at spawn) |
| `SETTLE_Y_PUSH_THRESH` | 0.28 m  | `|Y| > 0.28` means pushed into a bin (not just fell off end) |
...
| `'Y'`   | Yellow object detected (fallback, normally not used via camera) |
...
```python
SIM_STEPS_PER_FRAME = 10          # physics steps per viewer frame
TARGET_FRAME_TIME   = 10 × 0.002  # = 0.020 s  →  50 fps target
```
...
for _ in range(SIM_STEPS_PER_FRAME):
    mujoco.mj_step(model, data)           # 1. Advance physics
    manager.run_conveyor(                  # 2. Belt kinematic override
        excluded=push_sched.active_pushes)
    push_sched.step(data.time)            # 3. Fire/retract pushers
    manager.step(data.time)               # 4. Settle detection + spawn
    step_count += 1
    if step_count % VISION_EVERY_STEPS == 0:
        _vision_tick(...)                  # 5. Vision (every 5 steps)

viewer.sync()                              # 6. Render viewer frame
...
```python
viewer.cam.lookat   = [1.0, 0.0, 0.53]  # center of belt
viewer.cam.distance = 4.5               # ~4.5 m from target
viewer.cam.elevation = -20              # looking slightly down
viewer.cam.azimuth  = 160              # from the -Y side, slightly behind
```
...
<default class="flat_obj">
  <geom type="box" size="0.030 0.030 0.030"   <!-- 60×60×60 mm cube -->
        density="600"                           <!-- ~kg/m³, ~216 g per object -->
...
<default class="pusher_face">
  <geom friction="0.3 0.003 0.0001"   <!-- Low friction: object slides off cleanly -->
        condim="3"/>                    <!-- No spin friction needed for pusher -->
```
...
```python
BELT_VELOCITY = 0.200   # m/s — increase for faster throughput
```
...
```python
N_OBJECTS = 15   # Must match XML: add object_10 ... object_14 bodies
```
...
```python
PUSHER_RED_EXTENDED    = -0.30   # More negative = deeper push
...
```python
COLOR_MIN_PX = 60    # Lower → more sensitive, higher → fewer false positives
```

### Simulation Speed

```python
SIM_STEPS_PER_FRAME = 10   # Increase → faster sim (less real-time accurate)
                            # Decrease → slower, smoother at low frame rates
```
...
*Generated for the KGF65-KA Belt Conveyor MuJoCo Simulation project.*
